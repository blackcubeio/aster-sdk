...
| Méthode | Entrée | Sortie |
|---|---|---|
| `place(i)` | `PlaceOrderParams` | `Promise<Order>` *(snapshot complet de l'ordre)* |
| `cancel(i)` | `CancelOrderParams` | `Promise<void>` |
| `cancelAll(i)` | `CancelAllParams` | `Promise<{ cancelled: number \| null }>` |
| `edit(i)` | `EditOrderParams` | `Promise<{ name: string; id: string }>` *(identité seule, pas un `Order` — relire via `getOpens`)* |
...
| `withdraw(i)` | `WithdrawParams` | `Promise<WithdrawResult>` *(résultat brut — pas de forme commune cross-DEX ; Aster : `{ withdrawId, hash }`)* |
...
// Aster : `chainId`/`asset`/`fee` requis (validés avant l'appel réseau ; `fee` via getWithdrawFeeSpot).
await dex.account().withdraw({ amount: '100', chainId: '56', asset: 'USDT', fee: '0.8' });
...
Routes **Aster** : `wallet ↔ wallet` (perp↔spot, `transferFuturesSpot`, `asset` défaut `USDT`) ·
`to: { subAccount }` (`subAccountTransfer`, `from: { subAccount }` optionnel).

```ts
await dex.transfers().transfer({ from: { wallet: 'perp' }, to: { wallet: 'spot' }, amount: '10' });
await dex.transfers().transfer({ to: { subAccount: '0x…' }, asset: 'USDT', amount: '10' });
```
...
*(Noms alignés sur le code : suffixe `Params`. Les dates `startTime`/`endTime` sont des datetime UTC
`"YYYY-MM-DD HH:MM:SS"` à la frontière commune.)*
...
// Contrat ouvert : `amount` commun + champs requis du DEX via `[extra]`. Aster **exige**
// `chainId`, `asset`, `fee` (validés par la façade avant l'appel réseau).
...
// Résultat de retrait Aster (pas de forme commune cross-DEX — chaque DEX renvoie un ack différent).
interface WithdrawResult { withdrawId: string; hash: string }
