...
import type { CancelOrderParams, Order } from '../types';
import { buildOrderRef } from './payloads';

/** Cancel an active order (`TRADE`). */
export function cancelOrder(params: CancelOrderParams, label: string): Promise<Order> {
  ...
  return httpPostForm<Order>('futures', '/fapi/v3/order', body, network, 'DELETE');
}
